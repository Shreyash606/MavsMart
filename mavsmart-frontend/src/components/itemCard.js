...
const ItemCard = ({ item, isLoggedIn, navigate, handleDelete }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row items-stretch mb-4 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 w-full">
      {/* Product Image */}
      <div className="basis-1/3 w-full md:w-1/3 flex-shrink-0 overflow-hidden rounded-t-lg md:rounded-s-lg md:rounded-tr-none flex items-center justify-center bg-gray-100">
        <img
          src={item.photo}
          style={{ width: "300px", height: "300px" }}
          alt={item.title}
          className="w-full h-64 sm:h-48 object-contain object-center p-1"
          onError={(e) => {
            e.target.src = "";
          }}
        />
      ...
      <div className="flex flex-col md:basis-2/3 justify-between p-4 md:p-6 min-w-0 flex-1 min-h-[200px]">
        <div className="min-w-0">
          <div className="flex justify-between text-xs md:text-sm text-gray-500 gap-2">
            <h5 className="mb-2 text-lg sm:text-xl md:text-2xl font-bold text-gray-900 truncate">
              {item.title}
            </h5>
            {/* Delete Button */}
            {isLoggedIn && item.uploadedBy === auth.currentUser.displayName && (
              <TrashIcon
                ...
                className="w-5 h-5 sm:w-6 sm:h-6 text-gray-500 hover:text-red-500 cursor-pointer"
              />
            )}
          </div>
        ...
        <div className="space-y-3 min-w-0">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-2">
            ...
            <span className="flex-shrink-0 text-xs sm:text-sm font-semibold bg-blue-100 text-blue-800 px-2.5 py-1 rounded-full">
              {item.category}
            </span>
          </div>
          ...
          {/* Conditional Buttons */}
          <div className="transition-all duration-300 flex flex-col sm:flex-row gap-2 justify-end">
            {!showContact ? (
              <button
                type="button"
                onClick={handleBuyClick}
                className="text-white w-full sm:w-32 bg-[#0064b1] hover:bg-slate-500 hover:text-black focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 mb-2"
              >
                Buy Now
              </button>
            ) : (
